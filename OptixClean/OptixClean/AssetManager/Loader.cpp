...
void Loader::load(string fileName) {
	...
	string operation, values1, values2, values3, values4;
	...
	while (configStream >> operation) {
		if (operation.substr(0, 2).compare("//") == 0) {
			...
			if (values2.compare("1") == 0) {
				configStream >> values2;
				shaderString += " " + values2 + "\n";
			}
			else if (values2.compare("2") == 0) {
				configStream >> values2 >> values3;
				shaderString += " " + values2 + " " + values3 + "\n";
			}
			else if(values2.compare("3") == 0){
				configStream >> values2 >> values3 >> values4;
				shaderString += " " + values2 + " " + values3 + " " + values4 + "\n";
			}
			else {
				cout << "not understand operation " << operation << values1 << values2 << endl;
			}
		}
