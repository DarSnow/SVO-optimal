...
void Light::bindLight(Shader shader, int i) {
	char ts[20];
	_itoa_s(i, ts, 18);

	shader.setVec3((string("pointLights[") + ts + "].position").c_str(), vec3(transform.Position().x, transform.Position().y, transform.Position().z));
	shader.setVec3((string("pointLights[") + ts + "].ambient").c_str(), vec3(ambient.x, ambient.y, ambient.z));
	shader.setVec3((string("pointLights[") + ts + "].diffuse").c_str(), vec3(diffuse.x, diffuse.y, diffuse.z));
	shader.setVec3((string("pointLights[") + ts + "].specular").c_str(), vec3(specular.x, specular.y, specular.z));
	shader.setFloat((string("pointLights[") + ts + "].constant").c_str(), attenuation.Constant());
	shader.setFloat((string("pointLights[") + ts + "].linear").c_str(), attenuation.Linear());
	shader.setFloat((string("pointLights[") + ts + "].quadratic").c_str(), attenuation.Quadratic());
}
